...
const PainChart = ({ resident, setResidents, aiEntry }) => {
  ...
  useEffect(() => {
    ...
    const entry = {
      ...
      time: aiEntry.time || new Date().toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit", hour12: false }),
    };
        ...
        <Box sx={{ width: "100%", height: 150, mb: 3 }}>
          <ResponsiveContainer>
            <LineChart data={painEntries}>
              ...
              <YAxis domain={[0, 10]} />
              <Tooltip />
              <Line type="monotone" dataKey="painLevel" stroke="#2E7D32" strokeWidth={2} dot={{ r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        </Box>
      ) : (
